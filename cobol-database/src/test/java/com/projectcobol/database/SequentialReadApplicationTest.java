...
/**
 * Golden-output JUnit 5 test for {@link SequentialReadApplication}.
 *
 * <p>This is the largest and most complex of the four {@code cobol-database} test
 * classes. It exercises the Java translation of the COBOL {@code DISPLAY-DET-S}
 * paragraph from {@code OpenCobol/Database/SequentialRead.cbl} (lines 63-67), which
 * emits four {@code DISPLAY} lines per record: the full {@code DETAILS} structure,
 * {@code DETAILS-ID}, {@code DETAILS-NAME} (the surname only), and
 * {@code DETAILS-BIRTHDAY}. The production class reads the 36-record classpath
 * fixture {@code data.txt}, right-pads each line to the 30-character COBOL
 * {@code DETAILS} width, and extracts fixed-offset substring views, so the captured
 * output is exactly {@code 36 records × 4 lines = 144 lines} ({@code 3636} bytes).
 *
 * <p>The production {@link SequentialReadApplication#run(String...)} method is
 * invoked directly via {@code new SequentialReadApplication().run(new String[]{})}
 ...
 * {@code src/test/resources/expected/SequentialReadApplication.txt}.
 */
class SequentialReadApplicationTest {
    ...
    /**
     * Verifies that {@link SequentialReadApplication} prints exactly the 144 lines of
     * the golden fixture (36 records × 4 lines each), matching the behavior of the
     * original COBOL {@code DISPLAY-DET-S} paragraph
     * ({@code OpenCobol/Database/SequentialRead.cbl:L63-67}).
     *
     * @throws Exception propagated from {@link SequentialReadApplication#run(String...)}
    ...
    void producesExpectedGoldenOutput() throws Exception {
        ...
        new SequentialReadApplication().run(new String[]{});

        // Load the pre-recorded expected fixture from the test classpath.
        String expected = loadExpectedFixture("expected/SequentialReadApplication.txt");
     ...
     * relative resource path {@code expected/SequentialReadApplication.txt} resolves to
     * {@code cobol-database/src/test/resources/expected/SequentialReadApplication.txt}.
     ...
     *                     {@code expected/SequentialReadApplication.txt}
